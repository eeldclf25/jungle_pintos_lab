...
void thread_init(void)
{
	...
	/* Alarm Clock 을 위해 만든 sleep_list*/
...
/* priority 에 따라 내림차순 정렬. ready_list에 스레드를 삽입시 사용.*/
				  ...
				  void *aux UNUSED)
{
	...
	/* 더 높은 priority가 앞쪽에 오도록 함*/
	return (ta->priority) > (tb->priority);
}

/* wakeup_tick에 따라 올므차순 정렬. sleep_list에 스래드 삽입시 사용. */
...
/* 현재 실행주인 스레드를 주어진 wakeup_tick까지 잠들게(blocked 상태)로 전환하고 sleep_list에 넣는다.*/
...
/*current_tick을 기준으로, sleep_list에 있는 스레드 중 wakeup_tick이 지난 스데르를 깨워 ready_list로 보낸다.*/
void thread_wakeup(int64_t current_tick)
{
	...
	{
		/* list_elem 포인터를 struct thread 포인터로 변환시킨다.*/
		struct thread *t = list_entry(cur, struct thread, elem);

		/* 현재 스레드의 깨어날 시간이 현재 시간보다 작거나 같으면 깨운다.*/
		if (t->wakeup_tick <= current_tick)
		{
			/* 현재 요소 제거, 다음 요소 반환*/
			cur = list_remove(cur);

			/* 스레드를 ready_list에 넣고, 상태를 THREAD_READY로 바꾼다.*/
			thread_unblock(t);
		}
		else
		{
