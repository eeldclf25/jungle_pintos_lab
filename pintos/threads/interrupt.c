...
intr_enable(void)
{
	...
	// intr_context가 false일 땐 외부 인터럽트 처리 중이 아니라는 뜻
	// ASSERT는 조건이 거짓이면 종료하기 때문에 이 함수가 외부 인터럽트 컨텍스트에서 호출되면 안 된다는 의미
	ASSERT(!intr_context());
	...
	/* Enable interrupts by setting the interrupt flag.
	   ...
	   Hardware Interrupts".
	asm volatile C 코드에 인라인 어셈블리(기계어 명령어)를 삽입하는 문법
	"sti"는 x86 명령어로, 인터럽트 플래그(IF)를 1로 만들어 CPU가 인터럽트를 받을 수 있게(인터럽트 활성화)*/
	asm volatile("sti");
...
/* Sends an end-of-interrupt signal to the PIC for the given IRQ.
   ...
   주어진 IRQ에 대해 PIC(Programmable Interrupt Controller)에 인터럽트 종료 신호(EOI, End Of Interrupt)를 보낸다.
   만약 우리가 이 신호를 보내지 않으면, 해당 IRQ는 다시는 우리에게 전달되지 않으므로 매우 중요하다.
*/
static void
